 ...
 */
 ...
 class SearchPage {
    ...
    get productCount () { return $('//span[@class="item-count"]')}
    ...
    async selectFilterByText(text) {
        const filterElement = await $(`//span[@class='facet-option-label-text'][normalize-space()='${text}']`);
   ...
   /*  async selectProductBylinkText(ProductSKU) {
        var i = 0
        const ele = $$('sku-header')
        for(i = 0; i< ele.length; i++){
            const element = await ele([i]).getAttribute('href');
            if(await element.toInclude(`skuId=${ProductSKU}`)){
                await $(['href'=element]).click()
            }
        }
    } */
